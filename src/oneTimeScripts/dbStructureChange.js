...
async function createAllCharDocuments(collectionName) {
    ...
    for(let l = 0; l < allChars.length; l++) {
        ...
        const doc = await db.collection(collectionName).doc(dataTemp.char).get();   
        if (!doc.exists) {
            const setRes = await db.collection(collectionName).doc(dataTemp.char).set(dataTemp);
        } 
    }
}
...
async function writeToFirebase(dict, collectionName) {
    ...
    for (const [key, value] of Object.entries(dict)) {
        const writeRes = await db.collection(collectionName).doc(key).update({
            names: value
        }).then(function() {
